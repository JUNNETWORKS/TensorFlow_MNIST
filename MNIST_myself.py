...
"""
        # 保存modelが ない場合 → 学習をする / ある場合 → モデルを読み込み
        if tf.train.get_checkpoint_state('./ckpt'):
            ...
            img = cv2.imread(img, cv2.IMREAD_GRAYSCALE)  # 画像読み込み
            img = cv2.resize(img, (28,28))
            img = img.flatten().astype(np.float) # 1次元の784次元にする

            # 画像に対する予測ラベルの表示
            pred = sess.run(logits, feed_dict={x_image: img, keep_prob:1.0}) # 画像の数字を予測して各数字の確率を求め,最大確率の数字を返す
            print(pred)
"""
# 保存データがない場合
